var employees = [
    {
        name : "David",
        phone: "800-555-5555",
        address:    "123 Main St., Longmont, CO  80537"
...

    {
        name : "Bob",
        phone: "303-123-4567",
        address:    "1441 18th Street, Denver, CO  80202"
...
$(document).ready(function(){
    ...
    $("#add").click(function() {
        console.log('clicked!');
        render_edit_box('add');
        $("#edit_box").slideDown ('slow', function(){
...
function render_edit_box(type, employee, index){
    ...
    var html =
        '<div>' +
        '<label>name</label>' +
        '<input id="edit_name" value="' + pre_name + '">' + '</div>' +
        '<div><label>phone</label> ' +
        '<input id="edit_phone" value="'+ pre_phone + '">' + '</div>' +
        '<div>' +
        '<label>address</label>' +
        '<input id="edit_address" value="' + pre_address + '">' + '</div>'

    var button_name = type == "add" ? "add it" : "update it";

    html += "<button id='saveit'>"+ button_name + "</button>";

    $('#edit_box').html(html).slideDown();
...
function render_employee_table(data){
    ...
    data.forEach(function(employee, index){
        ...
        html += "<td><button class='delete' index='"+index+"'><img src= 'deleteButton.jpg'/></button></td>";
        html += "<td><button index="+ index +" class='edit'><img src= 'edit.jpg'/></button></td>";
